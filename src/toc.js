...
const createTocFromFolderP = function(pathParam, toc, rootPath) {
      ...
      prom = prom
        ...
        .then((stats) => {
          ...
          if (toc.items[id].isDir) {
            return createTocFromFolderP(fullPath, toc, rootPath).then((newToc) => {
              toc = newToc;
...
const mergeTocConfigWithToc = function(toc, tocConfig) {
...
const calculateItemLevels = function(toc, items, level) {
...
const getHtmlChapters = function(html) {
  const chapters = {
    root: [],
    items: []
  };

  const headingRegexStr = '<h(\\d*)(?: id="(.*?)")?.*?>([\\s\\S]*?)<\\/h\\1(?: .*?)?>';
  ...
  if (matches != null && matches.length > 0) {
    ...
    recursiveHeadings = pushHeadingsRecursive(matches);
...
module.exports = {
  getHtmlChapters,
  createTocFromFolderP,
  mergeTocConfigWithToc,
  calculateItemLevels
}
