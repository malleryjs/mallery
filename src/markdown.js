      ...
      var mutateItem = function(item) {
        if (item != null) {
          ...
          if (item.type == 'table') {
            var data = patch(item, 'data', {});
            var props = patch(data, 'hProperties', {});
            props.class = 'table table-responsive table-hover table-bordered';
          }
          if (item.children != null) {
...
const compileFileP = function(srcPath, destPath) {
  return readFileP(srcPath).then(remarkWithPluginsP).then(res => {
    if (destPath != null) {
      return writeFileP(destPath, res.contents)
        .then(() => res.contents );
    }
    else {
      return res.contents;
