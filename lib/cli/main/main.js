...
config.get().then(config => {
  ...
  const assetsPath = paths.project("lib/front/assets");
  ...
  const prepareOutput = function(config) {
    return util
      ...
      .then(() => {
        return shell.cp("-R", jsSrcPath, jsDestPath);
