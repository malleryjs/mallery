...
const envIsTesting = function() {
  return process.env.TEST === "1";
};

const envIsWatching = function() {
  return process.env.WATCH === "1";
};
...
const getDefinePlugin = ({ isNode, env, nameSpaceId, isLibrary }) => {
  const webpack = require("webpack");
  const isTesting = envIsTesting(env);
  ...
  if ((!isNode && !isLibrary) || nameSpaceId === "front") {
    variables["process.env.NODE_ENV"] = JSON.stringify(process.env.NODE_ENV);
  }
