...
const markdownP = require('./src/markdown').compileFileP;
const handlebars = require('handlebars');
const Toc = require('./src/toc');
const config = require('./src/config');
const pkg = require("./package.json");
const jsSrcPath = path.join(__dirname, 'dist', 'mallery.js');

config.get().then((config) => {
  const jsDestPath = path.join(config.paths.output, 'app.min.js');
  const cssSrcPath = path.join(__dirname, 'dist', 'mallery.css');
  const cssDestPath = path.join(config.paths.output, 'app.min.css');
  const assetsPath = path.join(__dirname, 'assets');
  const layoutSrcPath = path.join(assetsPath, 'templates', 'layout.hds.html');
  ...
  const prepareOutput = function(config) {
    return util.clearDirP(config.paths.output)
      .catch((err) => {
        // console.error(err);
      })
      .then(() => {
      ...
      .then(() => {
        return util.copyP(jsSrcPath, jsDestPath);
      })
      .then(() => {
  ...
  if (config.serve) {
    ...
    prepareOutput(config)
      ...
      .then(toc => {
          ...
          if (item.path != null) {
            ...
            if (indexItemId === idx) {
              ...
              if (item.name.indexOf('index') >= 0) {
                ...
                var newPath = newPath + item.name.replace('index', 'index' + idx);
                item.path = newPath;
              }
            }
        ...
        return util.readFileP(layoutSrcPath, 'utf-8')
          .then((layoutStr) => {
            return {
              layoutStr,
              toc
            ...
            prom = prom
              ...
              .then(() => {
                if (!item.isDir) {
                  ...
                  return util.mkdirP(path.dirname(item.absoluteOutputPath))
                    .then(() => {
                      return util.mkdirP(path.dirname(item.absoluteHtmlOutputPath));
                    });
                }
              });
          }
            ...
            if (!item.isDir) {
                ...
                extensions.forEach(extension => {
                  if (extension === item.ext ) {
                    prom2 = prom2.then(() => {
                      switch(type) {
                        case 'markdown':
                          return markdownP(item.absolutePath);
                          break;
                        case 'html':
                          return util.readFileP(item.absolutePath, 'utf-8');
                          break;
                        case 'plaintext':
                          return util.readFileP(item.absolutePath, 'utf-8')
                            .then((content) => {
              ...
              prom2 = prom2.then((content) => {
                if (content !== false && content != null) {
                  ...
                  return util.writeFileP(item.absoluteHtmlOutputPath, content)
                    .then(() => {
                      return util.writeFileP(item.absoluteOutputPath, content)
                    })
                } else {
      ...
      .then(config => {
        ...
        config.toc.items.forEach((item, itemId) => {
          if (item.hasContent) {
            fileCopyProm = fileCopyProm.then(() => {
              return util.readFileP(item.absoluteOutputPath, 'utf-8');
            })
            .then((content) => {
              config.toc.activeItemId = itemId;
              const tpl = handlebars.compile(config.layoutStr);
              ...
              return util.writeFileP(item.absoluteOutputPath, result)
            });
          }
        })
